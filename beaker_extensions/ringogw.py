...
class DecodeMulti:
    ...
    def output(self):
        if self.buf:
            raise ReplyException("%d extra bytes in the stream" % len(self.buf))
        else:
...
class Ringo:
    ...
    def request(self, url, data=None, verbose=False, retries=0, decoder=DecodeJson):
        ...
        if code == 408 and retries > 0:
            time.sleep(0.1)
            return self.request(url, data, verbose, retries - 1, decoder)
        else:
            return code, dec.output()

    def check_reply(self, reply):
        if reply[0] != 200 or reply[1][0] != "ok":
            e = ReplyException("Invalid reply (code: %d): %s" % (reply[0], reply[1]))
            e.retcode = reply[0]
            e.retvalue = reply[1]
            raise e
        return reply[1][1:]
